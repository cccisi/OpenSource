...
void hw_poly_ntt(poly *r)
{
  eff_ntt_CT_intt_GS(r->coeffs,0);
}

void hw_poly_invntt(poly *r)
{
  eff_ntt_CT_intt_GS(r->coeffs,1);
}
...
//    r->coeffs[i] = barrett_reduce(a->coeffs[i] + b->coeffs[i]);
    r->coeffs[i] = ((a->coeffs[i] * 7651) % 7681 + b->coeffs[i]) % 7681; //for HW test
}
